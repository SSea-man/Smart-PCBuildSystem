...
<div class="admin-layout-wrapper">
  ...
  <main class="admin-workspace">
    ...
    <div class="d-flex justify-content-between align-items-center mb-1 flex-wrap gap-2">
      <div>
        <h2 style="font-family: var(--font-head); font-weight:800; font-size:1.6rem; color:var(--text-primary); margin:0;">Component Catalogue</h2>
        <p class="text-muted small" style="margin:0;">Add, modify or delete physical hardware listings.</p>
      </div>
      <div class="d-flex gap-2">
        <form method="GET" class="d-flex gap-2">
          <select name="cat" class="form-select form-select-sm" onchange="this.form.submit()" style="border-radius:10px; width:150px;">
        ...
        <button class="btn btn-primary btn-sm" style="background:#7c3aed; border-color:#7c3aed; border-radius:12px; font-weight:600;" data-bs-toggle="modal" data-bs-target="#comp-modal">
            ...
            <?php else: ?>
              <?php foreach ($list as $c): ?>
                <tr>
                  ...
                  <td class="py-2">
                    <span class="badge bg-accent-soft text-accent" style="font-size:0.7rem; font-weight:600;"><?= sanitise(type_to_category($c['type'])) ?></span>
                  </td>
                  ...
                  <td class="py-2">
                    ...
                    ?>
                    <span class="badge <?= $stockClass ?>" style="font-size:0.7rem;"><?= sanitise($c['stock_raw']??'—') ?></span>
                  </td>
                  <td class="py-2" style="font-size:0.85rem; font-weight:600;"><?= number_format((float)$c['benchmark_score'], 0) ?></td>
                  <td class="pe-3 py-2 text-end">
                    <div class="d-inline-flex gap-1">
                      <a href="?action=edit&id=<?= $c['component_id'] ?>" class="btn btn-sm btn-outline-secondary" style="border-radius:8px; padding:0.2rem 0.5rem;" title="Edit">
                      ...
                      <form method="POST" class="d-inline" onsubmit="return confirm('Delete this component?')">
                        ...
                        <button type="submit" class="btn btn-sm btn-outline-danger" style="border-radius:8px; padding:0.2rem 0.5rem;" title="Delete">
    ...
    <div class="modal-content" style="background:var(--bg-card); border:1px solid var(--border); border-radius:16px;">
      ...
      <form method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          <div class="row g-3">
            <div class="col-md-8">
              ...
              <input type="text" name="name" class="form-control form-control-sm" value="<?= sanitise($edit['component_name']??'') ?>" required style="border-radius:8px;">
            </div>
            <div class="col-md-12">
            ...
            <div class="col-md-6">
              ...
              <input type="url" name="startech_url" class="form-control form-control-sm" placeholder="https://startech.com.bd/..." value="<?= sanitise($edit['startech_url']??'') ?>" style="border-radius:8px;">
            </div>
            <div class="col-md-6">
              <label class="form-label small fw-600">Ryans URL (Optional)</label>
              <input type="url" name="ryans_url" class="form-control form-control-sm" placeholder="https://ryanscomputers.com/..." value="<?= sanitise($edit['ryans_url']??'') ?>" style="border-radius:8px;">
            </div>
            <div class="col-md-4">
              ...
              <select name="type" class="form-select form-select-sm" required style="border-radius:8px;">
                <?php foreach ($categories as $ct): ?>
                  <option value="<?= $ct ?>" <?= ($edit['type']??'')===$ct?'selected':'' ?>><?= $ct ?></option>
                <?php endforeach; ?>
              </select>
            </div>
            <?php
            ...
            <div class="col-md-4">
              ...
              <select name="brand" class="form-select form-select-sm" required style="border-radius:8px;">
                ...
                <?php foreach ($brands as $b): ?>
                  <option value="<?= $b ?>" <?= ($edit['brand']??'')===$b?'selected':'' ?>><?= $b ?></option>
                <?php endforeach; ?>
              </select>
            </div>
            <?php
            ...
            foreach ($simple_fields as [$fn,$lbl,$tp]):?>
              <div class="col-md-4">
                ...
                <input type="<?= $tp ?>" name="<?= $fn ?>" class="form-control form-control-sm" value="<?= sanitise((string)($edit[$fn]??'')) ?>" step="any" style="border-radius:8px;">
              </div>
            <?php endforeach; ?>
          </div>
        </div>
        <div class="modal-footer" style="border-top:1px solid var(--border);">
          ...
          <button type="submit" class="btn btn-primary btn-sm" style="background:#7c3aed; border-color:#7c3aed; border-radius:8px;">Save Component</button>
        </div>
      </form>
    </div>
  </div>
